...
router.get('/stepuiconfig/model/:templateId', (req, res) => {
    ...
    skillController.getSkillModel(templateId).then(model => {
        res.send(model);
    }).catch(error => {
        res.status(500).send(ErrorUtil.attachErroInfo(error));
    });
...
router.get('/stepuiconfig/stepuistate/:taskId/:stepIndex', (req, res) => {
    ...
    skillController.getStepUIState(taskId, stepIndex).then(stepUIState => {
...
router.post('/stepuistate/:taskId/:stepIndex', (req, res) => {
    ...
    skillController.saveStepUIState(req.params.taskId, req.params.stepIndex, stepUIState).then(data => {
        res.send({
            status: "success"
        });
    }).catch(error => {
...
router.post('/xmlgeneration', (req, res) => {
    ...
    skillController.generateXML(templateId, taskId, stepIdx, stepText).then(msg => {
        res.send({
            status: "success"
        });
    }).catch(error => {
...
router.get("/resource/*", (req, res) => {
    ...
    skillController.getResource(filePath).then(resourceData => {
...
router.delete("/resource/*", (req, res) => {
    ...
    skillController.removeResourceFile(filePath).then(success => {
        res.send({
            "status": "success"
        });
    }).catch(error => {
