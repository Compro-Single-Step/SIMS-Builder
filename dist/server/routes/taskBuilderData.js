...
function validateTaskData(taskData) {
	...
	return getTemplatOptions().then(templateData => {
		...
		for (let stepIndex = 0; stepIndex < stepData.length; stepIndex++) {
			//function getSkillName to bre renamed to getTemplateName if Baloo gives skill name and set only template name in it..
	...
	return new Promise((resolve, reject) => {
		taskTemplateMapModel.getTaskMap(condition, projection, (error, data) => {
			...
			try {
				...
				if (err) resolve("NotSelected"); //cannot reject as promise.all in taskModel will fail in that case
...
router.post("/stepTemplate", function (req, res) {
	...
	taskTemplateMapModel.updateTaskMap(condition, updateData, options, (error, success) => {
		if (error) res.send(JSON.stringify({ "message": "DATABASE_ERROR" }));else updateStepData(step, templateId).then(step => {
